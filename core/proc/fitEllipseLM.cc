    ...
    cv::RotatedRect * rc)
{
        ...
        cv::Mat1d * jac, bool * have_analytical_jac) final
    {
      ...
      if (!jac ) {
        for( int i = 0; i < n; ++i ) {
          ...
          const double R_meas = std::sqrt(xr * xr + yr * yr);
          const double cos_theta = xr / R_meas;
          const double sin_theta = yr / R_meas;
          const double k = axis_ratio;
          const double D_norm = std::sqrt((k * cos_theta) * (k * cos_theta) + sin_theta * sin_theta);
          const double R_theor = (a * k) / D_norm;
          const double w = p.z;
        ...
        *have_analytical_jac = true;
        ...
        for( int i = 0; i < n; ++i ) {
          ...
          *rhsp++ = w * (R_meas - R_theor);
          ...
          // d(R_meas)/dx0 = -cos_theta * ca + sin_theta * sa;
          // d(R_meas)/dy0 = -cos_theta * sa - sin_theta * ca;
          const double dRmeas_dx0 = -cos_theta * ca + sin_theta * sa;
          const double dRmeas_dy0 = -cos_theta * sa - sin_theta * ca;
  ...
  c_lm_solver solver;
  solver.set_epsfn(1e-9);
  solver.set_epsx(1e-9);
        ...
        cv::Mat1d * jac, bool * have_analytical_jac) final
    {
      ...
      if (!jac ) {
        for( int i = 0; i < n; ++i ) {
          ...
          const double xr =  dx * ca + dy * sa;
          const double yr = -dx * sa + dy * ca;
          const double R_meas = std::max(1e-3, std::sqrt(xr * xr + yr * yr));
