...
struct FrameHessian
{
  ...
  float * absSquaredGrad[PYR_LEVELS] = { nullptr };  // only used for pixel select (histograms etc.). no NAN.

  int frameID = 0;						// incremental ID for keyframes only!
  int idx = 0;

  // Photometric Calibration Stuff
  float frameEnergyTH = 0;	// set dynamically depending on tracking residual
  float ab_exposure = 0;
...
struct PointHessian
{
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
  static int instanceCounter;
  EFPoint * efPoint = nullptr;

  // static values
  float color[MAX_RES_PER_POINT] = { 0 };			// colors in host frame
  float weights[MAX_RES_PER_POINT] = { 0 };		// host-weights for respective residuals.
