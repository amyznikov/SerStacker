    ...
    const c_align_color_channels_options & opts)
{
  ...
  //return estimate(_src, _srcmask, reference_channel, reference_mask, opts);
...
//  CF_DEBUG("c_align_color_channels: ENTER");
  ...
  std::vector<cv::Mat> channels;
  cv::Mat cumulative_image;
  cv::Mat1b cumulative_mask, dummy_mask;
  ...
  for( int i = 0; i < cn; ++i ) {
    ...
    if ( !_image_transforms[i] ) { // reference channel is not remapped
      continue;
    }

    if( opts.use_fixed_remap ) {
      if ( _fixed_remaps[0][i].size() != imageSize ) {
        _image_transforms[i]->create_remap_fixed(imageSize, _fixed_remaps[0][i], _fixed_remaps[1][i]);
      }
    }
    else {
      if( _remaps[i].size() != imageSize ) {
    ...
    if( _dst.needed() ) {
      if( opts.use_fixed_remap ) {
        cv::remap(channels[i], channels[i], _fixed_remaps[0][i], _fixed_remaps[1][i],
            opts.interpolation,
            opts.border_mode,
            opts.border_value[i]);
      }
      else {
        cv::remap(channels[i], channels[i], _remaps[i], cv::noArray(),
            opts.interpolation,
            opts.border_mode,
            opts.border_value[i]);
      }
    ...
    if( _dstmask.needed() ) {
      ...
      if ( !_srcmask.empty() ) {
        ...
        if ( dummy_mask.empty() ) {
          dummy_mask.create(imageSize);
          dummy_mask.setTo(255);
        }
        m = dummy_mask;
      }
