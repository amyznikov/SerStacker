    ...
    if ( _frame_accumulation ) {
      ...
      if ( true ) {
        ...
        if( _output_options.save_acc_weights ) {
          if( !_frame_accumulation->get_acc_counters(accw) ) {
    ...
    cv::Mat & reference_frame, cv::Mat & reference_mask)
{
  ...
  if( _output_options.debug_frame_registration ) {
    _frame_registration->set_debug_path(ssprintf("%s/debug/reference_frame",
        _output_path.c_str()));
  }
  ...
  if( _output_options.debug_frame_registration ) {
    _frame_registration->set_debug_path("");
  }

  if ( _output_options.debug_frame_registration && _frame_registration->options().enable_eccflow_registration  ) {

    const c_eccflow &eccflow =
        _frame_registration->eccflow();
    ...
    double * output_reference_timestamp)
{
  ...
  const bool save_raw_bayer_image =
      _master_options.accumulation.accumulation_method == frame_accumulation_bayer_average ||
      _master_options.accumulation.accumulation_method == frame_accumulation_bayer_drizzle;

  if( !read_input_frame(input_sequence, _input_options, reference_frame, reference_mask, is_external_master_file, save_raw_bayer_image) ) {
    ...
    if ( _frame_registration ) {
      ...
      if( _output_options.debug_frame_registration ) {
        ...
        if( _output_options.debug_frame_registration_frame_indexes.empty() ) {
          _frame_registration->set_debug_path(ssprintf("%s/debug/registration-%d",
          ...
          if( pos == _output_options.debug_frame_registration_frame_indexes.end() ) {
            ...
            _frame_registration->set_debug_path(ssprintf("%s/debug/registration-%d", _output_path.c_str(), *pos));
          }
        }
...
//            current_mask.depth() == CV_8U ? 1. / 255 : 1,
//            CV_32F);
//      }
//
    ...
    if ( _frame_accumulation ) {
      ...
      if( c_bayer_average *bayer_average = dynamic_cast<c_bayer_average*>(_frame_accumulation.get()) ) {
        ...
        CF_DEBUG("bayer_average: current_mask: %dx%d depth=%d",
            current_mask.cols, current_mask.rows,
            current_mask.depth());

        static const cv::Mat2f empty_remap;
        bayer_average->set_remap(_frame_registration ? _frame_registration->current_remap() : empty_remap);
        bayer_average->add(_raw_bayer_image, current_mask);


      }
      else if( c_bayer_drizzle *bayer_drizzle = dynamic_cast<c_bayer_drizzle*>(_frame_accumulation.get()) ) {
...
//      opts.sharpness_measure.kradius > 0;
//  return opts.accumulation_method == frame_accumulation_weighted_average &&
...
bool c_image_stacking_pipeline::upscale_required(frame_upscale_stage current_stage, bool generating_master_frame) const
{
  ...
  const c_frame_upscale_options & opts = _upscale_options;
...
//template<class RootObjectType>
//static inline void ctlbind_master_accumulation_options(c_ctlist<RootObjectType> & ctls,
//    const c_ctlbind_context<RootObjectType, c_image_stacking_master_options> & ctx)
//{
//  using S = c_image_stacking_master_options;
//  ctlbind(ctls, ctx(&S::accumulation));
//}
  ...
  if ( ctls.empty() ) {
    ...
    ctlbind_expandable_group(ctls, "3. Upscale options",
        [&, cctx = ctx(&this_class::_upscale_options)]() {
          ...
          ctlbind(ctls, "Stage:", CTL_CONTEXT(cctx, upscale_stage), "");
        });
    ...
    ctlbind_expandable_group(ctls, "7. Output options",
        [&, cctx = ctx(&this_class::_output_options)]() {
          ...
          ctlbind_expandable_group(ctls, "Save ACC Masks", "");
            ctlbind(ctls, "save_accumulation_masks",CTL_CONTEXT(cctx, save_accumulation_masks), "");
            ctlbind(ctls,CTL_CONTEXT(cctx, output_acc_masks_video_options));
          ctlbind_end_group(ctls);
          ...
          ctlbind_expandable_group(ctls, "Save Acc Weights", "");
            ctlbind(ctls, "save acc weights",CTL_CONTEXT(cctx, save_acc_weights), "");
          ctlbind_end_group(ctls);
      });
  }
...
bool c_image_stacking_pipeline::save_accumulation_masks_video(const cv::Mat & current_frame, const cv::Mat & current_mask, int seqindex)
{
  ...
  const c_output_frame_writer_options & output_opts = _output_options.output_acc_masks_video_options;
