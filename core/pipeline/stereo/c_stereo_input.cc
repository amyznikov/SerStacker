...
static bool read_input_frame(const c_input_source::sptr & source, const c_stereo_input_options & input_options,
    cv::Mat & output_image,
    cv::Mat & output_mask,
    cv::InputArray missing_pixel_mask = cv::noArray())
{
  INSTRUMENT_REGION("");
  ...
  if( input_options.enable_color_maxtrix && source->has_color_matrix() && output_image.channels() == 3 ) {
...
//  }
  ...
  if ( !missing_pixel_mask.empty() ) {

    if ( output_image.size() != missing_pixel_mask.size() ) {
      ...
      CF_ERROR("Invalid input: "
          ...
          missing_pixel_mask.cols(), missing_pixel_mask.rows());

      return false;
    }

    if ( output_mask.empty() ) {
      missing_pixel_mask.copyTo(output_mask);
    }
    else {
      cv::bitwise_and(output_mask, missing_pixel_mask,
          output_mask);
    }
  }
...
bool open_stereo_source(c_stereo_input_source & source, const c_stereo_input_options & input_options)
{
  ...
  if ( input_options.layout_type == stereo_frame_layout_separate_sources ) {
...
bool read_stereo_source(c_stereo_input_source & source,
    const c_stereo_input_options & input_options,
    cv::Mat output_frames[2],
    cv::Mat output_masks[2])
{
  switch (input_options.layout_type) {
    case stereo_frame_layout_separate_sources:
      ...
      for( int i = 0; i < 2; ++i ) {
        ...
        if( !read_input_frame(source.inputs[i], input_options, frame, mask) ) {
    ...
    case stereo_frame_layout_horizontal: {
      ...
      if( !read_input_frame(source.inputs[0], input_options, image, mask) ) {
