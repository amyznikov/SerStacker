/*
 * QAddDataProcRoutineDialogBox.cc
 *
...
QAddDataProcRoutineDialogBox::QAddDataProcRoutineDialogBox(QWidget * parent) :
  Base(parent)
{
  setWindowTitle("Select data processing routine");
  ...
  hbox2->addWidget(cancel_ctl = new QPushButton("Cancel", this));
...
void QAddDataProcRoutineDialogBox::populateClassList()
{
  classlist_ctl->clear();
  selectedClassFactory_ = nullptr;

  for ( const processor_class_factory * c : c_data_frame_processor_routine::class_list() ) {
...
void QAddDataProcRoutineDialogBox::onFilterTextChanged(const QString & filter)
{
  if ( filter.isEmpty() ) {
...
void QAddDataProcRoutineDialogBox::onClassListCurrentItemChanged(QListWidgetItem *current, QListWidgetItem *previous)
{
  if ( !current ) {
    tooltip_ctl->clear();
  }
  else {
    const processor_class_factory * c =
        static_cast<const processor_class_factory*>(current->data(Qt::UserRole).value<void*>());
...
void QAddDataProcRoutineDialogBox::onOkClicked()
{
  QListWidgetItem * current_item = classlist_ctl->currentItem();
  if ( current_item ) {
    selectedClassFactory_ =
        static_cast<const processor_class_factory *>(current_item->data(Qt::UserRole).value<void*>());
    accept();
  }
}

const c_data_frame_processor_routine::class_factory * QAddDataProcRoutineDialogBox::selectedClassFactory() const
{
