...
template<class T>
static void extract_keypoints_(const cv::Mat & img, const cv::Mat & mask, T threshold, std::vector<cv::KeyPoint> & keypoints)
{
  const cv::Mat_<T> src =
      img;
    ...
    for( int y = 0; y < src.rows; ++y ) {
      for( int x = 0; x < src.cols; ++x ) {
        if( src[y][x] > threshold ) {
          keypoints.emplace_back(cv::KeyPoint(x, y, 1, -1, src[y][x], 0, -1));
        }
      }
    }
...
static void extract_keypoints(const cv::Mat & img, const cv::Mat & mask, double threshold, std::vector<cv::KeyPoint> & keypoints)
{
  switch (img.depth()) {
    case CV_8U:
      return extract_keypoints_<uint8_t>(img, mask, (uint8_t) threshold, keypoints);
    case CV_8S:
      return extract_keypoints_<int8_t>(img, mask, (int8_t) threshold, keypoints);
    case CV_16U:
      return extract_keypoints_<uint16_t>(img, mask, (uint16_t) threshold, keypoints);
    case CV_16S:
      return extract_keypoints_<int16_t>(img, mask, (int16_t) threshold, keypoints);
    case CV_32S:
      return extract_keypoints_<int32_t>(img, mask, (int32_t) threshold, keypoints);
    case CV_32F:
      return extract_keypoints_<float>(img, mask, (float) threshold, keypoints);
    case CV_64F:
      return extract_keypoints_<double>(img, mask, (double) threshold, keypoints);
  }
}
...
void c_morph_features_extractor::detect(cv::InputArray _src, std::vector<cv::KeyPoint> & keypoints, cv::InputArray mask)
{
  ...
  extract_keypoints(lap, mask.getMat(), options_.threshold, keypoints);
}
