...
void c_lpg_map_routine::getcontrols(c_control_list & ctls, const ctlbind_context & ctx)
{
  ...
  ctlbind_item(ctls, "Copy c_lpg_options to clipboard", ctx, [](this_class * _ths) {
    return ctlbind_copy_config_to_clipboard("c_lpg_options", _ths->_m.options()), false;
  });
  ctlbind_item(ctls, "Paste c_lpg_options from clipboard", ctx, [](this_class * _ths) {
