...
void QDataFrameRoutineOptionsControl::setupControls()
{
  ...
  std::vector<struct c_data_processor_routine_ctrl> params;
  ...
  for( const struct c_data_processor_routine_ctrl &p : params ) {

    switch (p.ctl_type) {

      case c_data_processor_ctl_begin_group: {
        ...
        currentGroup =
            currentSettings->add_expandable_groupbox(p.name.c_str(),
      ...
      case c_data_processor_ctl_end_group: {
      ...
      case c_data_processor_ctl_flags_chkbox: {

        QFlagsEditBoxBase * ctl =
            currentSettings->add_flags_editbox_base(p.name.c_str(),
                p.tooltip.c_str(),
                p.get_enum_members(),
                [this, p](int v) {
      ...
      case c_data_processor_ctl_numeric_box: {

        QWidget * ctl =
            currentSettings->add_numeric_box<std::string>(p.name.c_str(),
                p.tooltip.c_str(),
      ...
      case c_data_processor_ctl_enum_combobox: {

        QWidget * ctl =
            currentSettings->add_enum_combobox_base(p.name.c_str(),
                p.tooltip.c_str(),
                p.get_enum_members(),
                [this, p](int v) {
                  if (!updatingControls() && p.set_value && p.set_value(toString(v))) {
      ...
      case c_data_processor_ctl_check_box: {

        QWidget * ctl =
            currentSettings->add_checkbox(p.name.c_str(),
                p.tooltip.c_str(),
                [this, p](bool checked) {
      ...
      case c_data_processor_ctl_browse_for_existing_file: {

        QWidget * ctl =
            currentSettings->add_browse_for_path(p.name.c_str(),
                "",
      ...
      case c_data_processor_ctl_browse_for_directory: {
      ...
      case c_data_processor_ctl_spinbox: {

        QSpinBox * ctl =
            currentSettings->add_spinbox(p.name.c_str(),
                p.tooltip.c_str(),
                [this, p](int v) {
                  if (!updatingControls() && p.set_value && p.set_value(toString(v)) ) {
                    Q_EMIT parameterChanged();
      ...
      case c_data_processor_ctl_double_slider : {

        QDoubleSliderSpinBox * ctl =
            currentSettings->add_sliderspinbox<double>(p.name.c_str(),
                p.tooltip.c_str(),
                [this, p](double v) {
                  if (!updatingControls() && p.set_value && p.set_value(toString(v)) ) {
                    Q_EMIT parameterChanged();
      ...
      case c_data_processor_ctl_math_expression: {

        QInputMathExpressionWidget * ctl =
            currentSettings->add_math_expression(p.name.c_str(),
                p.tooltip.c_str(),
                [this, p](const QString & v) {
                  if (!updatingControls() && p.set_value && p.set_value(v.toStdString()) ) {
                    Q_EMIT parameterChanged();
