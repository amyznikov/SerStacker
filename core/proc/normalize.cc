    ...
    const cv::Scalar & unmaskedPixelsValue)
{
  ...
  if ( mask.empty() || (mask.channels() == 1 && cv::countNonZero(mask) == mask.size().area()) ) {
    ...
    tbb::parallel_for(range(0, dst.rows, grain_size),
        [&dst, &msk, im, om, S, fillUnmaskedPixels, unmaskedPixelsValue ](const range & r ) {
          for ( int y = r.begin(), ymax = r.end(); y < ymax; ++y ) {
            cv::Vec<T, cn> * dstp = dst[y];
            ...
            for ( int x = 0, n = dst.cols; x < n; ++x ) {
              ...
              else if ( fillUnmaskedPixels ) {
                dstp[x] = unmaskedPixelsValue[0];
              }
            }
          }
        });
    ...
    tbb::parallel_for(range(0, dst.rows, grain_size),
        [&dst, &msk, imin, omin, S, fillUnmaskedPixels, unmaskedPixelsValue ](const range & r ) {
          for ( int y = r.begin(), ymax = r.end(); y < ymax; ++y ) {
            cv::Vec<T, cn> * dstp = dst[y];
    ...
    const cv::Scalar & unmaskedPixelsValue)
{
  ...
  if ( mask.empty() || (mask.channels() == 1 && cv::countNonZero(mask) == mask.size().area()) ) {
    ...
    tbb::parallel_for(range(0, dst.rows, grain_size),
        [&dst, &msk, S, imin, omin, omax, cn, fillUnmaskedPixels, unmaskedPixelsValue](const range & r ) {
          for ( int y = r.begin(), ymax = r.end(); y < ymax; ++y ) {
            T * dstp = dst.ptr<T>(y);
            const uint8_t * mskp = msk[y];
    ...
    tbb::parallel_for(range(0, dst.rows, grain_size),
        [&dst, &mask, S, imin, omin, omax, cn, fillUnmaskedPixels, unmaskedPixelsValue](const range & r ) {
          for ( int y = r.begin(), ymax = r.end(); y < ymax; ++y ) {
