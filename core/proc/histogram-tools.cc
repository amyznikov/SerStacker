...
void autoClip(const cv::Mat1d & H, double realMinValue, double realMaxValue, double * lclip, double * hclip)
{
  ...
  if( H.cols == 1 ) {
    makeCumulativeHistogram(H, Hc, true);
  }
  else {
    cv::reduce(H, Hc, 1, cv::REDUCE_AVG);
    makeCumulativeHistogram(Hc, Hc, true);
  }

  cv::Scalar lb, hb;
  computeHistogramClipLevels(Hc, realMinValue, realMaxValue, 1e-3, 1.0 - 1e-5, lb, hb);
  *lclip = lb[0];
  *hclip = hb[0];
}
