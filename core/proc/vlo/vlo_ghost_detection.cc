 ...
 *  */
template<class EchoType, class _Cb> std::enable_if_t<vlo_scan_has_peak_member(EchoType()),
...
static inline bool vlo_ghost_detection_(const ScanType & scan,
    const c_vlo_ghost_detection_options & opts,
    cv::Mat & output_ghsosts_mask)
{
  typedef typename ScanType::Echo echo_type;

  output_ghsosts_mask.create(scan.NUM_LAYERS, scan.NUM_SLOTS, CV_8UC3);
  ...
  get_vlo_echos(scan,
      [&](int l, int s, const echo_type echos[3]) {
