...
//static bool _project_to_radius_vector(const cv::Mat & _gx, const cv::Mat & _gy, const cv::Point2f & rp,
...
//  if( _gr ) {
//    _gr->create(rows, cols, CV_MAKETYPE(CV_32F, cn));
//  }
//
//  if( _gt ) {
//    _gt->create(rows, cols, CV_MAKETYPE(CV_32F, cn));
//  }
//
//  const cv::Mat_<float> gx = _gx;
//  const cv::Mat_<float> gy = _gy;
//
//  cv::Mat_<float> gr = _gr ? *_gr : cv::Mat_<float>();
//  cv::Mat_<float> gt = _gt ? *_gt : cv::Mat_<float>();
//
...
//        float * __restrict grp = _gr ? gr[y] : nullptr;
//        float * __restrict gtp = _gt ? gt[y] : nullptr;
//
...
//            if ( grp ) {
//              grp[x * cn + c] = (+gxv * ca + gyv * sa);
//            }
//            if ( gtp ) {
//              gtp[x * cn + c] = (-gyv * ca + gxv * sa);
//            }
//          }
//        }
//      }
...
bool c_radial_gradient_routine::process(cv::InputOutputArray image, cv::InputOutputArray mask)
{
  ...
  switch (_output_type) {
    case OutputRadialGradient:
      project_to_radius_vector(_reference_point, gx, gy, g, cv::noArray());
      break;
    case OutputTangentialGradient:
      project_to_radius_vector(_reference_point, gx, gy, cv::noArray(), g);
      break;
  }
